...
function filterDestinations() {
    ...
    const searchTerm =
        destinationSearch.value.trim().toLowerCase();
...
const modalGroupSize =
    document.getElementById('modal-group-size');
...
if (
    ...
    destination.departures.forEach(departure => {
        ...
        option.textContent =
            `${formatDepartureDate(start)} → ${formatDepartureDate(end)} (${availableSeats} seats available)`;
...
tripDeparture.addEventListener(
    'change',
    () => {
        ...
        if (
            ...
            bookingTravelers =
                Math.max(1, availableSeats);

            travelerCount.textContent =
                bookingTravelers;
...
increaseTravelers.addEventListener(
    ...
    () => {
        ...
        if (!selectedDeparture) {
            ...
            alert(
                'Please select a departure first.'
            );
...
if (bookingTravelers > availableSeats) {
    ...
    destinationId:
    destinationData[currentDestination]._id,
...
viewMyBooking.addEventListener('click', () => {
    const bookingReference =
        bookingReferenceNumber.textContent.trim();
