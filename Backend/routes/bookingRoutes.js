  ...
  try {
    ...
    if (authHeader && authHeader.startsWith("Bearer ")) {
      try {
        ...
        console.log("Optional user authentication failed.");
      }
    }
    ...
    if (!destinationId || !departureId || !travelers) {
      return res.status(400).json({
        message: "Destination, departure and travelers are required.",
      });
    }
    ...
    if (
      ...
      return res.status(400).json({
        message: "Invalid destination or departure.",
      });
    }
    ...
    await session.withTransaction(async () => {
      ...
      const destination =
        await Destination.findById(destinationId).session(session);
      ...
      const departure = destination.departures.id(departureId);

      if (!departure) {
        throw new Error("Selected departure is no longer available.");
      }
      ...
      bookingDeparture = {
        ...
        availableSeats: departure.totalSeats - departure.bookedSeats,
      };
    });
  ...
  try {
      ...
      if (status === "Cancelled" && oldStatus !== "Cancelled") {
        ...
        if (booking.departureId) {
          const destination = await Destination.findOne({
            "departures._id": booking.departureId,
          }).session(session);

          if (destination) {
            const departure = destination.departures.id(booking.departureId);

            if (departure) {
              departure.bookedSeats = Math.max(
                0,
                departure.bookedSeats - booking.travelers,
              );
    ...
    if (shouldSendConfirmationEmail) {
      try {
        ...
        console.log(`Status email sent to ${booking.customerEmail}`);
      } catch (emailError) {
...
router.post("/confirm-paid-booking", async (req, res) => {
  const session = await mongoose.startSession();
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        message: "Required booking and payment details are missing.",
      });
    }
    ...
    sendBookingEmail(booking, "confirmed")
      .then(() => {
        console.log(
          `Booking confirmation email sent to ${booking.customerEmail}`,
        );
      })
      .catch((emailError) => {
    ...
    console.error("Paid booking confirmation failed:", error);
    ...
    return res.status(500).json({
      message: "Failed to confirm paid booking.",
    });
  } finally {
    await session.endSession();
  }
